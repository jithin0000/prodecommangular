...
import {
  ...
  filterProduct, filterProductSuccess, filterProductFailure
} from '../actions';
...
export const _product_reducer = createReducer(
  ...
  on(createProductFailure, (state, action) => ({
    ...state, loading: false, loaded: false, error: true,
    error_message: action.payload.toString()
  })),

  on(filterProduct, (state, action) => ({...state, loading: true, loaded: false})),
  on(filterProductSuccess, (state, action) => {
  ...
  on(filterProductFailure, (state, action) => ({...state, loading: false, loaded: false, error: action.payload}))
);
